...
export function FormPreview() {
  ...
  function onSubmit(data: z.infer<typeof validationSchema>) {
    ...
    toast.success(<pre className="whitespace-break-spaces break-all">{JSON.stringify(formattedData, null, 2)}</pre>);
  }
    ...
    <DrawerContent>
      <div className="mx-auto w-full max-w-lg">
        <DrawerHeader>
          ...
          {form.description && <DrawerDescription>{form.description}</DrawerDescription>}
        </DrawerHeader>

        <Form {...formHook}>
          <ScrollArea className="h-[50vh]">
            <form onSubmit={formHook.handleSubmit(onSubmit)} className="space-y-6 px-4">
